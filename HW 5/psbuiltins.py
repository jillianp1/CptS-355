...
class Stacks:
    def __init__(self, scoperule):
        ...
        # You should store the scope argument value in a local attribute in the `Stacks` 
        # object (for example `scope`) 
    ...
    def lookup(self,name):
        ...
        #If the scoping rule is dynamic, perform lookup by searching the AR’s (tuples) top to down.
        #If it is static, use static-links for the search. 
        Name = '/' + name
        if self.scope == 'static':
            for (i, d) in (self.dictstack):
                if Name in d.keys():
                    return d[Name]

        else: #its dynamic
            for (i, d) in reversed(self.dictstack):
                if Name in d.keys():
                    return d[Name]
    ...
    def getinterval(self):
        ...
        if(isinstance (string, StrConstant)):
            ...
            origstring = list(string.value)
        
            stringcopy = []
            for i in range(len(origstring)):
                stringcopy.append(origstring[i])
            #set up slice
            ...
            stringcopy2 = "(" + stringcopy + ")"
            self.opPush(StrConstant(stringcopy2))
        else:
