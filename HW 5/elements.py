...
class Name(Expr):
    ...
    def eval(self,psstacks):
        """TO-DO (part2)"""
        #check if zero index is a back slash then its evaluated as string
        if self.var_name[0] == '/':
            psstacks.opPush(str(self.var_name))
        #if its a built in operator 
            ...
            if isinstance(value, CodeArray):
                #find index function look for index where its been defined 
                # get the index static link 
                #index = self.getIndex(self.var_name)
                index = self.getIndex((psstacks.dictstack), self.var_name)
                #pass index and empty dict push it call apply then pop
    ...
    #helper for eval 
    def getIndex(self, stack, name):
        newName = '/' + name
        topIndex = (len(stack) -1)
        if newName in stack[topIndex][1]:
            return topIndex
        else:
            link = stack[topIndex][0]
            while link != 0:
                if newName in stack[link][1]:
                     return link
                else:
                    link = stack[link][0]
